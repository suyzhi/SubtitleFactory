...
export interface TranscriptionModelStatus {
  ...
  runtimes?: {
    ...
    repository?: string; revision?: string | null; source_url?: string | null;
...
export async function prepareTranscriptionModel(modelId: string, runtime: string, repair = false): Promise<{
  task_id: string; model_id: string; runtime: string; message: string;
}> {
  return request(`/api/transcription/models/${encodeURIComponent(modelId)}/prepare`, {
    method: 'POST', body: JSON.stringify({ runtime, repair }),
  });
}
...
export interface PlaybackInfo {
  ...
  duration: number | null;
  frame_rate_reliable: boolean;
  frame_rate_source: 'average_rate' | 'fallback_30fps';
}

export async function getPlaybackInfo(projectId: string): Promise<PlaybackInfo> {
  return request(`/api/projects/${projectId}/playback-info`);
}
