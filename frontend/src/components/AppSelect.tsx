...
export default function AppSelect({value,onChange,options,label,placeholder='请选择',searchable=false,allowCustom=false,disabled,className=''}:Props){
  ...
  const themeClass=themeHost.classList.contains('theme-dark')?'theme-dark':'theme-light';
  const playerPopover=Boolean(rootRef.current?.closest('.player-subtitle-menu'));
  ...
  const shell=<div ref={shellRef} className={`app-select app-select-shell ${themeClass} ${className} ${open?'expanded':''} ${position.above?'above':''} ${pressed?'pressed':''}`} style={open?{position:'fixed',left:position.left,width:position.width,...(position.above?{bottom:position.top}:{top:position.top})}:undefined}
