...
import type { EditorOperationResponse, TaskStatus } from '../types';
...
export default function SmartToolsPanel({ projectId, revision, duration, onEditorResult, onProjectChanged }: Props) {
  ...
  useEffect(() => () => { activeTaskId.current = ''; }, []);
  ...
  async function pollTask(taskId: string, onSuccess?: () => void | Promise<void>) {
    try {
      const current = await api.getTaskStatus(taskId);
      if (activeTaskId.current !== taskId) return;
      setTask(current);
      if (current.status === 'success') {
        const preview = current.details?.ocr_preview;
        if (Array.isArray(preview)) setOcrCues(preview as api.OCRCue[]);
        if (current.type === 'speaker_diarization') {
          await client.invalidateQueries({ queryKey: ['speakers', projectId] });
          onProjectChanged();
        }
        await onSuccess?.();
        return;
      }
      if (['failed', 'cancelled', 'partial'].includes(current.status)) {
        setMessage(current.error || current.message);
        return;
      }
      window.setTimeout(() => void pollTask(taskId, onSuccess), 800);
    } catch (error) {
  ...
  function monitorTask(taskId: string, onSuccess?: () => void | Promise<void>) {
    activeTaskId.current = taskId;
    setTask(null);
    void pollTask(taskId, onSuccess);
  }
  ...
  async function prepareManagedModels() {
    try {
      ...
      monitorTask(created.task_id, async () => { await managedModels.refetch(); });
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); }
  }
