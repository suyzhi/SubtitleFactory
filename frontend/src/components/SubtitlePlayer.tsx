...
const SubtitlePlayer = forwardRef<SubtitlePlayerHandle, Props>(function SubtitlePlayer({
  ...
  const seekAndRefresh = useCallback(async (next: number) => {
    ...
    await new Promise<void>(resolve => {
      ...
      video.addEventListener('seeked', presented, { once: true });
  ...
  useEffect(() => {
    ...
    api.getPlaybackInfo(projectId).then(info => {
      if (!activeRequest) return;
      setFrameDuration(info.frame_duration > 0 ? info.frame_duration : 1 / 30);
      setFrameRateReliable(info.frame_rate_reliable);
    }).catch(() => {
      if (!activeRequest) return;
      setFrameDuration(1 / 30);
      setFrameRateReliable(false);
  ...
  return (
    <div className={`pro-player ${controlsVisible ? 'controls-visible' : 'controls-hidden'} ${fullscreen ? 'player-fullscreen' : ''}`} ref={wrapperRef}
      ...
      onKeyDown={event => {
        ...
        if (event.key.toLowerCase() === 'r') { event.preventDefault(); void replay(); }
        if (event.key === ',') { event.preventDefault(); stepFrame(-1); }
        if (event.key === '.') { event.preventDefault(); stepFrame(1); }
        if (event.key.toLowerCase() === 't') {
          event.preventDefault();
          onPresentationModeChange(theaterMode ? 'normal' : 'theater');
        }
      ...
      {showSubtitleMenu && <div className="player-subtitle-menu" role="dialog" aria-label="字幕显示设置">
        ...
        <label>字幕位置 <span>{style.verticalPosition}%</span>
          <input type="range" min={SUBTITLE_POSITION_MIN} max={SUBTITLE_POSITION_MAX} value={style.verticalPosition}
