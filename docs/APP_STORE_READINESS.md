...
- 项目库首屏主 JavaScript 从 `535.79 kB / gzip 171.64 kB` 降至 `396.69 kB / gzip 118.15 kB`；设置中心、播放器、内容中心、播放列表和批量生产工具改为按需加载，构建不再出现 500 kB 主块警告。
...
- AI 凭据读取会在瞬时 Keychain 失败后重试一次；连续失败仍显示可恢复提示，不阻塞本地转写和其他设置。
