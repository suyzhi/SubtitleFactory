...
def _repair_invalid_settings(settings: dict[str, Any]) -> tuple[dict[str, Any], list[dict]]:
    ...
    warnings: list[dict] = []
    if (
        repaired.get("default_model") != "auto"
        and repaired.get("default_model") not in _MODEL_IDS
        and not _is_registered_model(repaired.get("default_model"))
    ):
        warnings.append({
    ...
    if repaired.get("yt_dlp_path"):
        warnings.append({
            "field": "yt_dlp_path",
            "code": "LEGACY_SETTING_IGNORED",
            "message": "旧版 yt-dlp CLI 路径已保留，但下载固定使用 App 内置 Python API",
            "fallback": "bundled_python_api",
        })
...
def read_ai_provider_cards():
    ...
    return {"providers": list_provider_cards(False), "assignments": {
        ...
        "content_provider_id": settings.get("content_provider_id") or "deepseek",
    }}
...
def update_ai_assignments(req: AIAssignmentsUpdate):
    try:
        get_provider(req.clean_provider_id, False); get_provider(req.translate_provider_id, False); get_provider(req.content_provider_id, False)
        settings = save_app_settings(req.model_dump())
        return {"assignments": {
            "clean_provider_id": settings["clean_provider_id"],
            "translate_provider_id": settings["translate_provider_id"],
            "content_provider_id": settings["content_provider_id"],
        }}
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
def test_ai_provider_card(provider_id: str):
    ...
    try:
        payload = prepare_chat_payload(provider, {"model": provider["model"], "messages": [{"role": "user", "content": "Reply with OK."}], "temperature": 0, "max_tokens": 8})
        response = httpx.post(f"{provider['base_url'].rstrip('/')}/chat/completions", headers={"Authorization": f"Bearer {provider['api_key']}", "Content-Type": "application/json"}, json=payload, timeout=30)
        raise_for_provider_status(response); response.json()["choices"][0]["message"]["content"]
    except Exception as exc:
...
def test_ai_connection(req: AIConnectionTest):
    ...
    started = time.monotonic()
    try:
        provider = {"provider": req.provider, "model": req.model}
        response = httpx.post(
            f"{base_url}/chat/completions",
            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
            json=prepare_chat_payload(provider, {
                "model": req.model,
